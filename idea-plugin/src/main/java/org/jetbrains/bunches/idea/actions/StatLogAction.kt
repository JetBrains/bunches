...
class StatLogAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (repoPath == null) {
            Messages.showMessageDialog("No git root found", "StatLog error", Messages.getErrorIcon())
            return
        }
        val bunchPath = File(BunchFileUtils.bunchPath(project), ".bunch")
        if (!bunchPath.exists()) {
            Messages.showMessageDialog("No .bunch file found", "StatLog error", Messages.getErrorIcon())
            return
        }
        ServiceManager.getService(project, BunchToolWindowFactory::class.java)
            .createStaLogActionWindow(window, project, repoPath)
    }
}